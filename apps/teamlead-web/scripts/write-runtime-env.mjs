 ...
 *
 * Only a fixed allowlist of VITE_* keys is emitted, and only when non-empty, so an
 * unset variable cleanly falls back to the build-time/localhost default. /env.js is
 * public (served to browsers) — never add non-VITE secrets to the allowlist.
 */
