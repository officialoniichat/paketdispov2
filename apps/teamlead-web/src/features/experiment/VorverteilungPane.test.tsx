...
describe('VorverteilungPane', () => {
  ...
  it('leerer Starter-Zustand: „Kommende Starter anschauen" klappt die Gruppen-Ansicht auf', () => {
    ...
    renderPane();
    fireEvent.click(screen.getByRole('button', { name: 'Starterbündel' }));
    expect(screen.getByText(/Kein Schichtstart in der nächsten Stunde/)).toBeTruthy();
    fireEvent.click(screen.getByRole('button', { name: 'Kommende Starter anschauen' }));
    // Mitarbeiterliste ist im Test leer → ehrlicher Leer-Hinweis statt Gruppen.
    expect(screen.getByText('Keine geplanten Schichtstarts in den nächsten 7 Tagen.')).toBeTruthy();
    expect(screen.getByRole('button', { name: 'Kommende Starter ausblenden' })).toBeTruthy();
  });
