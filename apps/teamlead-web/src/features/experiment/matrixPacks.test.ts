...
function bc(caseId: string, status: BoardCase['status'], totalQuantity = 10): BoardCase {
  return {
    caseId,
    weBelegNo: `WE-${caseId}`,
    status,
...
describe('derivePacks', () => {
  ...
  it('Belege ohne Pack-Zugehörigkeit bilden das Schluss-Pack „Manuell"', () => {
    const packs = derivePacks([bc('a', 'assigned'), bc('m', 'assigned')], [['a']]);
    expect(packs.map((p) => p.label)).toEqual(['Pack 1', 'Manuell']);
    expect(packs[1]!.cases.map((c) => c.caseId)).toEqual(['m']);
  });
